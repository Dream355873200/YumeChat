...
FramelessMainWindow::FramelessMainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    // 注释掉 WA_TranslucentBackground，使用Windows原生阴影机制
    // 注意：不能同时使用 Qt的透明背景 和 Windows的分层窗口
    ...
    // 启用Windows原生阴影
    enableWindowsShadow(true);
#endif
}

void FramelessMainWindow::enableWindowsShadow(bool enable)
{
#ifdef Q_OS_WIN
    HWND hwnd = (HWND)this->winId();
...
void FramelessMainWindow::paintEvent(QPaintEvent *event)
{
    ...
    // 设置背景颜色，确保窗口内容正确显示
    p.fillRect(rect(), QColor(255, 255, 255)); // 白色背景
